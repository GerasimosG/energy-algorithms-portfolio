...
python -m coverage erase && for f in tests/test_*.py; do python -m pytest "$f" -m "not slow and not pc" --cov=energy_algorithms --cov-append --cov-report= --cov-fail-under=0 -q || exit $?; done && python -m coverage report --fail-under=90  # RAM-bounded coverage gate
