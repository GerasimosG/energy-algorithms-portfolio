"""Tests for industry_demo — Industry Trading Demo functions.
...
def test_load_env_key_no_env_file(monkeypatch, tmp_path):
    ...
    import energy_algorithms.application.industry_demo as ed
    ...
    monkeypatch.setattr(ed, "__file__", str(fake_app_dir / "industry_demo.py"))
...
def test_load_env_key_with_env_file(monkeypatch, tmp_path):
    ...
    # So if __file__ = /tmp/.../application/industry_demo.py
...
def test_load_cached_data_missing_file(monkeypatch):
    ...
    from energy_algorithms.application.industry_demo import load_cached_data
...
def test_load_cached_data_with_csv(monkeypatch, tmp_path):
    ...
    # If __file__ = /tmp/.../src/energy_algorithms/application/industry_demo.py
...
def test_load_cached_data_csv_with_no_data_file(monkeypatch, capsys):
    ...
    fake_app_dir = tmp_path = os.path.join(os.sep, "tmp", "pytest-fake-nonexistent-dir")
    ...
    try:
        ...
        monkeypatch.setattr(ed, "__file__", os.path.join(fake_dir, "industry_demo.py"))
...
def test_fetch_recent_data_returns_prices(monkeypatch):
    ...
    from energy_algorithms.application.industry_demo import fetch_recent_data
...
def test_find_gas_marginal_day_high_price():
    ...
    from energy_algorithms.application.industry_demo import find_gas_marginal_day
...
def test_run_pcr_with_co2_no_co2():
    ...
    from energy_algorithms.application.industry_demo import run_pcr_with_co2
...
def test_main_runs_without_crash(monkeypatch, capsys):
    ...
    from energy_algorithms.application import industry_demo
    ...
    monkeypatch.setattr(industry_demo.os.path, "exists", mock_exists)

    industry_demo.main()
    captured = capsys.readouterr()
    assert "INDUSTRY TRADING DEMO" in captured.out
...
def test_main_with_cached_data(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(industry_demo, "_load_env_key", lambda: "")
    monkeypatch.setattr(
        industry_demo, "load_cached_data", lambda: (prices, dates, [])
    )
    # Avoid any client creation
    monkeypatch.setattr(industry_demo, "EntsoeClient", lambda **kw: None)
    # Mock the fetch_recent_data since no client
    monkeypatch.setattr(industry_demo, "fetch_recent_data", lambda c, **kw: ([], []))
...
def test_main_prints_trading_sections(monkeypatch, capsys):
    ...
    assert "INDUSTRY ROLE READINESS SUMMARY" in captured.out
