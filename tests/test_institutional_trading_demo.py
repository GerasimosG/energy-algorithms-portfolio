"""Tests for trading_demo — Energy Trading Demo functions.
...
def test_load_env_key_no_env_file(monkeypatch, tmp_path):
 ...
 monkeypatch.setattr(ed, "__file__", str(fake_app_dir / "institutional_trading_demo.py"))
...
def test_load_env_key_with_env_file(monkeypatch, tmp_path):
 ...
 # So if __file__ = /tmp/.../application/institutional_trading_demo.py
...
def test_load_cached_data_with_csv(monkeypatch, tmp_path):
 ...
 # If __file__ = /tmp/.../src/energy_algorithms/application/institutional_trading_demo.py
...
def test_load_cached_data_csv_with_no_data_file(monkeypatch, capsys):
 ...
 monkeypatch.setattr(ed, "__file__", os.path.join(fake_dir, "institutional_trading_demo.py"))
...
def test_main_runs_without_crash(monkeypatch, capsys):
 ...
 monkeypatch.setattr(trading_demo.os.path, "exists", mock_exists)

 trading_demo.main()
 captured = capsys.readouterr()
 assert "TARGET_ROLE TRADING DEMO" in captured.out
...
def test_main_with_cached_data(monkeypatch, capsys, tmp_path):
 ...
 monkeypatch.setattr(trading_demo, "_load_env_key", lambda: "")
 monkeypatch.setattr(
 trading_demo, "load_cached_data", lambda: (prices, dates, [])
 )
 # Avoid any client creation
 monkeypatch.setattr(trading_demo, "EntsoeClient", lambda **kw: None)
 # Mock the fetch_recent_data since no client
 monkeypatch.setattr(trading_demo, "fetch_recent_data", lambda c, **kw: ([], []))
...
def test_main_prints_trading_sections(monkeypatch, capsys):
 ...
 assert "TARGET_ROLE ROLE READINESS SUMMARY" in captured.out
