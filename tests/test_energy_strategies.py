...
def test_hour_of_day_in_sample_flag() -> None:
    """With no reference_profile, the strategy falls back to in-sample (flagged)."""
    prices = np.array([
        10, 10, 10, 10, 10, 10,
        90, 90, 90, 90, 90, 90,
        10, 10, 10, 10, 10, 10,
    ...
    _, meta = hour_of_day_strategy(prices, threshold_pct=0.05)

    assert meta["in_sample"] is True
    # An in-sample anchor is profitable by construction → every trade wins.
    assert meta["win_rate"] == 1.0


def test_hour_of_day_causal_can_lose() -> None:
    """A lagged profile that mispredicts produces a loss — proving no look-ahead."""
    # Reference profile (from prior days) says hour 0 is the cheapest → long it.
    reference = np.full(24, 50.0)
    reference[0] = 10.0
    # But today hour 0 is actually the most expensive → that long loses money.
    today = np.full(24, 50.0)
    today[0] = 90.0

    _, meta = hour_of_day_strategy(today, reference_profile=reference, threshold_pct=0.05)

    assert meta["in_sample"] is False
    assert meta["long_hours"] == 1          # only hour 0 flagged cheap by the profile
    assert meta["total_pnl_per_mwh"] < 0    # bought the priciest hour → a loss
    assert meta["win_rate"] < 1.0


def test_rolling_reference_profiles_first_none_then_lagged() -> None:
    """First day has no history (None); later days average the prior days."""
    days = [np.full(24, float(v)) for v in (10.0, 20.0, 30.0)]

    profiles = rolling_reference_profiles(days, lookback_days=7)

    assert profiles[0] is None
    assert np.allclose(profiles[1], 10.0)   # mean of day 0
    assert np.allclose(profiles[2], 15.0)   # mean of days 0 and 1
...
def test_energy_backtest_hour_of_day_aggregation() -> None:
    """Hour-of-day aggregation uses the per-MWh P&L keys (not a wrong, always-zero key)."""
    rng = np.random.default_rng(42)
    daily_prices = [rng.uniform(20, 80, 24).tolist() for _ in range(5)]
    dates = [f"2025-01-{d:02d}" for d in range(1, 6)]
    ...
    assert "avg_daily_pnl_per_mwh" in hod
    assert "total_pnl_per_mwh" in hod
    assert "std_pnl_per_mwh" in hod
    assert "avg_win_rate" in hod
    assert "best_day" in hod
    assert "worst_day" in hod
    assert "daily_details" in hod
    assert len(hod["daily_details"]) == 5
    # Aggregation must read the real per-day key: before the fix it read a
    # non-existent key ("total_pnl_pct") and silently produced 0.0.
    assert hod["total_pnl_per_mwh"] != 0.0
