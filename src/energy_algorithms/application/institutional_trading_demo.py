#!/usr/bin/env python3
"""Energy Trading Demo — energy algorithmic trading on real ENTSO-E data.

Demonstrates skills relevant to energy algorithmic trader / quant roles:
...
def main():
    print("=" * 72)
    print("  ENERGY TRADING DEMO — Energy Algorithmic Trading on Real Data")
    print("=" * 72)
    ...
    # Causal evaluation: trade each day on a profile built from PRIOR days only,
    # then settle at that day's realized prices (no look-ahead).
    hod_days = [np.array(p[:24]) for p in prices_list if len(p) >= 24]
    hod_profiles = rolling_reference_profiles(hod_days, lookback_days=7)
    hod_results = []
    for day_prices, profile in zip(hod_days, hod_profiles):
        if profile is None:
            continue  # first day has no prior history → cannot trade causally
        _, meta = hour_of_day_strategy(
            day_prices, reference_profile=profile, threshold_pct=0.03
        )
    ...
    # ── Section 5: Energy Readiness Summary ────────────────────────
    print(f"\n{'=' * 72}")
    print("  📊 ENERGY ROLE READINESS SUMMARY")
    print(f"{'=' * 72}")
    print("")
    ...
    print(f"{'─' * 72}")
    print("  💡 KEY INSIGHT FOR ENERGY INTERVIEWS:")
