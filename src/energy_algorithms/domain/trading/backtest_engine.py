...
def backtest(
    ...
    Returns
    -------
    dict with keys: equity_curve, trades, total_return, ann_return,
...
def tracked_backtest(
    ...
    *,
    ...
    """Run backtest wrapped with ML experiment tracking.

    Uses the default ``ExperimentTracker`` singleton. Logs all params
    and key metrics (Sharpe, return, drawdown, win rate) automatically.

    Example::

        result = tracked_backtest(prices, signals,
            experiment_name="sma_tuning",
    ...
    tracker = get_tracker()
    with tracker.run(name=experiment_name) as run:
        if params:
            for k, v in params.items():
                run.log_param(k, v)
        result = backtest(prices, signals, initial_capital, commission, slippage)
        for metric in ("sharpe", "total_return", "ann_return", "volatility",
                       "max_drawdown", "win_rate", "n_trades"):
            if metric in result:
                run.log_metric(metric, result[metric])
        return result
